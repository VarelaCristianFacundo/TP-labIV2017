	...
	class Reserva
	{
		...
		//--METODO DE CLASE
			public static function TraerUnaReservaPorId($id){
				...
				$sql = "SELECT U.id, U.id_usuario, U.local, U.fecha, U.cantidad
						FROM reservas U
						WHERE U.id = :id";
			...
			public static function Agregar($reserva){
				$conexion = self::CrearConexion();
				$sql = "INSERT INTO reservas (id_usuario, local, fecha, cantidad)
						VALUES (:id_usuario, :local, :fecha, :cantidad)";
				$consulta = $conexion->prepare($sql);
				$consulta->bindValue(":id_usuario", $reserva->id_usuario, PDO::PARAM_INT);
				$consulta->bindValue(":local", $reserva->local, PDO::PARAM_INT);
			...
			public static function Modificar($reserva){
				$conexion = self::CrearConexion();
				$sql = "UPDATE reservas
						SET id_usuario = :id_usuario, local = :local, fecha = :fecha, cantidad = :cantidad
						WHERE id = :id";
				$consulta = $conexion->prepare($sql);
				$consulta->bindValue(":id_usuario", $reserva->id_usuario, PDO::PARAM_INT);
				$consulta->bindValue(":local", $reserva->local, PDO::PARAM_INT);
